...
import torch
import matplotlib#jm

from data.getDataSet_02 import SimulatedAPTDataset

matplotlib.use('Agg')#jm
...
def draw_plt(list, ylabel):
    ...
    plt.grid()
    # plt.show()
    plt.savefig(f'{ylabel}_curve.png')#jm
    plt.close()#jm
...
if __name__ == '__main__':
    ...
    if dataset == "streamspot":
        streamspot_dataset_dir = r".\data\streamspot"
        ...
        if only_train_generated_mul:
            ...
            normal_cnt = 500
            # 修改正常数据集初始化
            normal_dataset = streamspot_dataset.NormalStreamSpotDataset(
                streamspot_dataset_dir,
                database_tar="all.tsv"  # 显式指定为tsv文件
            )
            # 同理修改恶意数据集和生成数据集
            malicious_dataset = streamspot_dataset.MaliciousStreamSpotDataset(
                streamspot_dataset_dir,
                database_tar="all.tsv"
            )
            generated_dataset = streamspot_dataset.GeneratedStreamSpotDataset(
                streamspot_dataset_dir,
                database_tar="all.tsv"
            )
            ...
            train_cnt = 1400
            test_cnt = 200
            original_dataset = streamspot_dataset.OriginalStreamSpotDataset(r".\data\streamspot")  # 600
            generated_dataset = streamspot_dataset.GeneratedStreamSpotDataset(streamspot_dataset_dir)  # 1000

...
# 暂定按照分钟按照分钟进行边的截取，计算下来，大约每分钟约有120k条边，每台主机大约50k边，边与节点的比例约为49：1 也即1k节点数
...
class DarpaCadetsDataset(torch_geometric.data.Dataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None, database_log_list=None,
        ...
        if database_log_list is None:
            # 关键修改：使用模拟数据集的事件JSON文件（替换真实数据集文件）
            database_log_list = ["darpa-tc-cadets-event.json"]
    ...
    def _generated_contextual(graph):
        ...
        contextual_graph, _ = gen_contextual_outlier(contextual_graph, n=100, k=50)
    ...
    def _generated_structural(graph):
        ...
        structural_graph, _ = gen_structural_outlier(structural_graph, m=10, n=10)
    ...
    def _save_tensor(self, graph, graph_filepath):
        ...
        # 确保processed目录存在
        os.makedirs(os.path.dirname(graph_file_abs_path), exist_ok=True)
    ...
    def get(self, idx):
        return torch.load(os.path.join(self.processed_dir, self.processed_info.get_filepath_from_list(idx)),weights_only= False)
...
class DarpaNormalDataset(DarpaCadetsDataset):
    ...
    def get(self, idx):
        return torch.load(os.path.join(self.processed_dir, self.processed_list[idx]),weights_only= False)